...
class robot:
    ...
    def __init__(self, name):
        ...
        while cond == 0:
            try:
                ...
                (trans, rot) = self.listener.lookupTransform(
                    self.global_frame, self.name + '/' + self.robot_frame,
                    rospy.Time(0))
                cond = 1
        ...
        self.make_plan = rospy.ServiceProxy(self.name + self.plan_service,
                                            GetPlan)
        ...
        for i in range(start, end + 1):
            if (i >= 0 and i < limit and i < len(mapData.data)):
                for j in range(0, len(centroids)):
                    ...
                    if (mapData.data[i] == -1 and
                            norm(point_of_index(mapData, i) - current_pt) <= r
                            and norm(point_of_index(mapData, i) - assigned_pt)
                            <= r):
